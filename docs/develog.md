...
- Added MVP UI and art direction: night pharmacy tree hollow, CSS-first scene, subtle visual tokens and symbolic card/astro visuals。
...
- UI direction: 夜間藥局 x 安靜樹洞 x 成人療癒感。
- Art strategy: CSS-first MVP visuals, no runtime image generation, generated assets only after inspection gate。
- MVP app: one-screen tree hollow input, pharmacist-specific mock response, embedded 30-second micro tool, song/card/astro panels and local saved items。
...
1. Review app copy with 3-5 pharmacist scenarios。
...
5. Collect anonymous ratings for 有被理解 and 有藥師感。
...
pass: `npm test` passed with 12 tests across safety, content and response behavior。

pass: `npm run build` passed for the Web PWA。

pass: 375px visual screenshot check passed for first screen, normal response and crisis response。
