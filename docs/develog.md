...
pass: 2026-06-15 production deploy fix passed `npx vitest run api/_lib/__tests__/respond-handler.test.ts`, `npx tsc -p api/tsconfig.json --noEmit`, `npm test`, and `npm run build`; production smoke tests returned `source: "safety"` for crisis and medical-boundary inputs after redeploy.
