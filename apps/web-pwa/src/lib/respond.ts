...
const scenarioKeywords: Array<{ tag: ScenarioTag; words: string[] }> = [
  { tag: "customer_conflict", words: ["客人", "民眾", "家屬", "罵", "兇", "客訴", "奧客"] },
  { tag: "prescription_overload", words: ["處方很多", "處方", "趕不完", "爆量", "排隊", "忙到"] },
  { tag: "interaction_worry", words: ["交互作用", "漏掉", "審方", "把關", "相互作用"] },
  { tag: "team_doubt", words: ["醫師", "同事", "主管", "質疑", "不相信"] },
  { tag: "pgy_pressure", words: ["PGY", "考核", "報告", "學長姐", "老師", "考試"] },
  { tag: "night_shift", words: ["夜班", "輪班", "大夜", "小夜", "睡不著"] },
  { tag: "shortage_pressure", words: ["缺藥", "沒有藥", "斷貨", "替代藥"] },
  { tag: "inventory_control", words: ["慢箋", "管制藥", "庫存", "盤點", "稽核"] },
...
function contextCopy(scenario: ScenarioTag): Pick<ConversationResponse, "message" | "empathy" | "praise" | "tinyAction"> {
  const copy: Record<ScenarioTag, Pick<ConversationResponse, "message" | "empathy" | "praise" | "tinyAction">> = {
    customer_conflict: {
      message: ["剛剛被那樣兇，心裡一定很不舒服。", "你還能把說明講完，已經很不簡單。"],
      empathy: "剛剛那種被情緒直接砸過來的感覺，真的很消耗。",
      praise: "你還能維持說明和確認，這不是忍耐而已，是專業在撐住邊界。",
      tinyAction: "先把肩膀放下來，讓剛剛那個聲音離你遠一點。"
    },
    prescription_overload: {
      message: ["處方一直進來的時候，真的會像整個人被推著跑。", "你不是只有在趕件數，你是在每一張裡面守住安全。"],
      empathy: "處方一張接一張時，很容易覺得自己被整座山壓住。",
      praise: "你不是只是在趕速度，你是在高壓裡一張一張守安全。",
      tinyAction: "現在先只看下一張，不看全部。"
    },
    interaction_worry: {
      message: ["會一直擔心漏掉交互作用，代表你其實很在意那條安全線。", "這種繃不是你太脆弱，是責任真的很重。"],
      empathy: "擔心交互作用漏掉，代表你腦中一直繃著安全那條線。",
      praise: "你會擔心，是因為你真的在把關，不是因為你不夠好。",
      tinyAction: "把擔心放回 SOP、工具和團隊，不讓自己一個人扛。"
    },
    team_doubt: {
      message: ["被質疑的那一下，會讓人很想立刻證明自己。", "但你有回到依據和流程，這比硬撐面子更專業。"],
      empathy: "被同事或醫師質疑時，專業感很容易被刺到。",
      praise: "你願意確認、溝通和守住依據，這本身就是專業的一部分。",
      tinyAction: "先把那句質疑和你的價值分開。"
    },
    pgy_pressure: {
      message: ["PGY 最累的地方，是好像每一天都在被看見、被評分。", "你還願意學、願意問，這已經是在往前。"],
      empathy: "PGY 的壓力常常不是單一事件，是一直被評分和比較。",
    ...
    shortage_pressure: {
      message: ["一直被追問缺藥，煩是很正常的。", "你接住的不是一個問題，是很多人的焦慮。"],
      empathy: "一直被問缺藥，像是一直替系統限制接住焦慮。",
      praise: "你能說明、查詢和交代可行選項，已經是在混亂裡維持秩序。",
      tinyAction: "先分清楚：能查的是你的工作，缺貨不是你的錯。"
    },
    inventory_control: {
      message: ["慢箋、管制藥、庫存這些東西，會把人逼到一直不能鬆。", "你願意一格一格確認，是在替流程守門。"],
      empathy: "慢箋、管制藥和庫存壓力會讓人一直處在不能出錯的狀態。",
...
export function buildResponse(input: string, mood: MoodTag): ConversationResponse {
  ...
  if (safety.riskLevel === "medical_boundary") {
    return {
      ...
      message: ["這個情境很有壓力，而且確實需要回到正式判斷流程。", "我不能替你判斷處方，但可以先陪你把那個緊繃放下一點。"],
      empathy: "這聽起來是需要專業判斷和流程支持的高壓場景。",
      praise: "你有停下來確認，而不是硬著頭皮處理，這是在保護病人安全。",
      tinyAction: "具體處方或用藥決策請回到院內 SOP、主管、資深藥師或正式藥品資訊；這裡先陪你把壓力放下來一點。",
